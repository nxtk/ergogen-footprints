...
Params:
  ...
  include_model_{mcu,socket}: default is false
  ...
  model_{mcu,socket}_filename: defaults are below
    Allows you to specify the path to a 3D model file relative to models_dir.
    supported formats (step, stpz, wrl, wrz, x3d, idf, emn)
       mcu - 'mcu_nicenano.step'
    socket - 'mcu_nicenano_socket.step'
  model_{mcu,socket}_{offset,rotation,scale}: default is [x, y, z] an array of decimal numbers
...
module.exports = {
  params: {
    ...
    include_labels: true,
    include_labels_on_both_sides: true,
    include_via_labels: true,
    ...
    // 3dmodels [x, y, z]
    include_model_mcu: false,
    include_model_socket: false,

    models_dir: '../../footprints/ceoloide/3dmodels/',
    model_mcu_filename: 'mcu_nicenano.step',
    model_mcu_offset: [0, 0, 0],
    model_mcu_rotation: [0, 0, 0],
    model_mcu_scale: [1, 1, 1],
    model_socket_filename: 'mcu_nicenano_socket.step',
  ...
  (segment (start ${p.eaxy(3.099695, -0.725 + (row_num * 2.54) - 12.7)}) (end ${p.eaxy(2.594695, -0.22 + (row_num * 2.54) - 12.7)}) (width 0.25) (layer "B.Cu"))
    ...
    const gen_socket_row = (row_num, pin_name_left, pin_name_right, include_via_labels, include_labels) => {
    ...
    (pad "" smd custom (at 5.5 ${-12.7 + row_offset_y} ${180 + p.r}) (size 0.2 0.2) (layers "B.Cu" "B.Paste" "B.Mask") ${p.local_net(socket_hole_num_right).str}
      ...
      if (p.include_silkscreen && include_labels) {
        if (p.reversible || p.include_labels_on_both_sides || p.side == 'F') {
    ...
    (fp_text user "${net_silk_front_right}" (at ${p.reversible && (row_num < 4 || !p.only_required_jumpers) ? (net_silk_front_right.length > 2 ? 1.45 : 2.04) : 4.47} ${-12.7 + row_offset_y} ${p.r}) (layer "F.SilkS")
      (effects (font (size 1 1) (thickness 0.15))))
        ...
        if (p.reversible && !p.include_extra_pins || p.include_labels_on_both_sides || p.side == 'B') {
    ...
    (fp_text user "${net_silk_back_right}" (at ${p.reversible && (row_num < 4 || !p.only_required_jumpers) ? (net_silk_back_right.length > 2 ? 1.45 : 2.04) : 4.47} ${-12.7 + row_offset_y} ${p.r}) (layer "B.SilkS")
      ...
      if (p.include_silkscreen && include_via_labels && (p.reversible && (row_num < 4 || !p.only_required_jumpers))) {
        socket_row += `
    ${''/* Via Labels - Front */}
    ...
    const gen_socket_rows = (include_via_labels, include_labels) => {
      const pin_names = [
      ...
      for (let i = 0; i < pin_names.length; i++) {
        ...
        const socket_row = gen_socket_row(
          ...
          include_via_labels, include_labels
        )
    ...
    (fp_line (start ${invert_pins ? '' : '-'}6.29 -11.43) (end ${invert_pins ? '' : '-'}8.95 -11.43) (layer "F.SilkS") (stroke (width 0.12) (type solid)))
        ...
        if (p.reversible || p.include_labels_on_both_sides || p.side == 'B') {
          socket_rows += `
  ...
  (footprint "ceoloide:mcu_nice_nano"
    ...
    const socket_rows = gen_socket_rows(
      p.include_via_labels, p.include_labels
    )
    ...
    `
    const model_mcu = `
    (model ${p.models_dir + p.model_mcu_filename}
      (hide ${p.include_model_mcu ? 'no' : 'yes'})
      (offset (xyz ${p.model_mcu_offset[0]} ${p.model_mcu_offset[1]} ${p.model_mcu_offset[2]}))
      (scale (xyz ${p.model_mcu_scale[0]} ${p.model_mcu_scale[1]} ${p.model_mcu_scale[2]}))
      (rotate (xyz ${p.model_mcu_rotation[0]} ${p.model_mcu_rotation[1]} ${p.model_mcu_rotation[2]})))
    `
    ...
    ${p.reversible && p.include_instructions && p.include_silkscreen ? instructions : ''}
    ${model_mcu}
    ${model_socket}
  )
